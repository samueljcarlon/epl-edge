...
def store_totals_all_lines(con, events: list[dict], captured_at: str) -> int:
    """
    Stores ALL totals lines found in the Odds API response for each bookmaker.
    Each (fixture_id, bookmaker, market=totals, line) becomes one row with over/under prices.
    """
    cur = con.cursor()
    rows = []
    ...
    for ev in events:
        ...
        for bm in ev.get("bookmakers", []):
            ...
            for mk in bm.get("markets", []):
                ...
                # Build map: line -> {"over": price, "under": price}
                line_map: dict[float, dict[str, float]] = {}
                ...
                for out in mk.get("outcomes", []):
                    ...
                    if line not in line_map:
                        line_map[line] = {}
                    if name == "over":
                        line_map[line]["over"] = pr
                    elif name == "under":
                        line_map[line]["under"] = pr

                for line, d in line_map.items():
                    over_price = d.get("over")
                    under_price = d.get("under")
...
def main() -> None:
    ...
    events = fetch_odds(
        ...
        markets=s.odds_markets,  # keep this as "totals" for now
        odds_format=s.odds_format,
        date_format=s.date_format,
    )
    n_odds = store_totals_all_lines(con, events, captured_at)
