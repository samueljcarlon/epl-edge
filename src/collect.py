...
def fetch_odds_base(
    ...
    # Debug for 4xx/5xx without printing secrets
    if r.status_code >= 400:
        ...
        print("ODDS_BASE_BODY", r.text[:500])
...
def fetch_event_odds(
    ...
    If rejected (401/403/422/429), skip without killing the run.
    """
    r = requests.get(
    ...
    if r.status_code >= 400:
        ...
        print("ODDS_EVENT_BODY", r.text[:500])
        return None
...
def store_btts_snapshots(con, base_events: list[dict[str, Any]], captured_at: str, settings) -> int:
    ...
    Mapping for our table shape:
      Yes -> over_price
      No  -> under_price
      line -> NULL (we store 0.0 to keep your schema happy, adjust if you want NULL)
    """
    rows: list[tuple[Any, ...]] = []
